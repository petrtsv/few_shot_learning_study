...
class DFMNLoss(nn.Module):
    ...
    def forward(self, query_features: torch.Tensor, labels: torch.Tensor, global_classes_mapping: dict,
                n_way: int) -> torch.Tensor:
        featmap_size_sq = self.in_featmap_size ** 2
        ...
        indices = torch.tensor(indices, device=prototypes.device)
